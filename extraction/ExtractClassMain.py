...
class ExtractClass:
    def extract(self, filename, pdf_path, output_folder):
        ...
        for i, page in enumerate(pdf_document):
            ...
            try:
                ...
                if self.should_skip_page(text, page_number):
                    continue  # Do not increment page_number if the page is skipped
    ...
    def should_skip_page(self, text, page_number):
        # This method can be overridden by subclasses to apply different skipping criteria
        
        if "scratch work" in text or "blank page" in text or len(text.strip()) < 5:
            print(f"Skipping page {page_number} due to content criteria")
            return True
        return False
...
def process_head_folder(head_folder_path, output_head_folder):
    ...
    for course_folder in os.listdir(head_folder_path):  # First loop: Iterate over course folders
        ...
        if os.path.isdir(course_path):
            ...
            output_course_folder = os.path.join(output_head_folder, "Math_Images_Questions_New", course_folder)
...
# Example usage
head_folder_path = r'C:\Users\Hemil Patel\Desktop\Questra DB\Questions_PDF_New'  # Math_PDFs
